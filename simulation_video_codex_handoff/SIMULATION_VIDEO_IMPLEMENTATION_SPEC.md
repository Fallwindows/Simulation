# SUPERSEDED HANDOFF COPY — DO NOT USE OR INSTALL

The canonical specification is `../SIMULATION_VIDEO_IMPLEMENTATION_SPEC.md` in
the repository root, together with `../EXECUTION_OVERRIDES.md`, `../AGENTS.md`,
and `../agent_roles/`. This retained transfer copy contains obsolete Astra/Luna
routing and 4K defaults. It is historical input only and must not overwrite the
canonical files.

# Grocery-store software demonstration — implementation specification (historical copy)
...
> This is an implementation assignment, not a request to return another plan. Read the specification, establish the real starting state, configure and prove the worker/reviewer workflow, then implement and render through the internal acceptance gates. Use the owner's chat for important decisions and independent review. Do not substitute planning documents, a mock video, or an attractive still for the working deliverables.

## Reading map

| Reader | Read first | Then load only what the task needs |
|---|---|---|
| Astra orchestrator | Sections 0–4, 15–22; `agent_roles/README.md`; `orchestrator_agent.md` | All shot requirements, architecture, quality goals, current evidence and decisions |
| Luna programmer | Shared role README; `programmer_agent.md`; task packet | Relevant sections of 5–14 and 16; assigned references and current code |
| Dedicated Sol reviewer | Shared role README; `reviewer_agent.md`; exact task/submission | Relevant contracts, acceptance criteria, full task diff, callers, independent evidence |
...
Section 23 lists primary-source entry points. Appendix A embeds the existing role documents so this Markdown remains sufficient to recover the role instructions when supplied alone. The companion handoff archive additionally contains those role files separately and all 12 reference images. Images are not embedded as bytes inside this Markdown.
...
Follow platform instructions and applicable repository instructions. Within project decisions, the owner's latest explicit directions supersede earlier suggestions; this specification supplies the agreed defaults. Original storyboard pixels govern visual composition and treatment. Actual declared data governs technical numbers and capability claims. Older audits and code comments are investigation leads, not facts to believe without verification.
...
| Resolution | Native 3840 × 2160, 16:9, square pixels; no upscale masquerading as native rendering |
...
| Delivery copies | High-quality 4K MP4; lightweight review MP4; silent master/version; selected lossless full-resolution frames |
...
Native 4K and long render times are acceptable. Extra samples are not a substitute for good materials or correct geometry. Benchmark representative frames, choose a converged quality profile, and estimate total wall time and disk needs before a long run. A duration estimate is not grounds to silently lower quality.
...
### 2.2 Runtime assumptions to verify

Historical project documentation identifies a Windows workstation, RTX 4070 Ti, Isaac Sim 6.1, native ROS 2 Jazzy through Pixi, RTAB-Map, and PowerShell launchers. These are leads, not a current hardware inventory. Discover exact GPU/VRAM, RAM, free disk, drivers, Python environments, simulator/API version, native binaries, encoding tools, and actual source-to-installed package relationships.

Do not assume a previous 16 GB RAM figure is still true. Do not assume the user's Mac is the render machine. Do not upgrade drivers, rebuild the whole native stack, migrate to WSL/Linux, or replace Isaac with a different engine without a concrete verified need and a focused decision.
...
|---|---|---|---|
| Main orchestrator | GPT-6 Astra | Medium | Task boundaries, dependency decisions, scheduling, integration, goal/evidence state, GitHub checkpoints |
| General programmer | GPT-5.6 Luna | High; medium only for routine organizational work | Implementation, self-tests, asset preparation, bounded experiments, local commits, every required revision |
...
| Technical implementer when necessary | Separate GPT-5.6 Sol session | High | Difficult implementation Luna cannot productively resolve; never approves its own change |
| Independent visual reviewer | GPT-5.6 Luna | High | Inspect actual frames/clips against the relevant reference and visual criteria; no edits to its reviewed candidate |
...
Treat abundant Luna capacity and access to the owner's chat as the owner's cost assumptions, not guaranteed product quotas. Spend Astra tokens on judgment and integration, not duplicated investigations or routine code. Do not silently route Luna work to Astra, downgrade Sol review to Luna, or pretend an agent is using a model its runtime did not expose.

Start with three active Luna coding tasks, one dedicated Sol reviewer, and bounded additional read-only/visual assignments when independent. One heavy GPU process at a time initially. More than two ready candidates waiting for Sol triggers backpressure: prioritize reviews and revisions rather than spawning more code writers. The dedicated reviewer may suspend when idle; no polling loops consuming tokens.

### 3.2 Install actual role instructions and verify routing

Read `agent_roles/README.md` and the relevant role. These files are supplied separately in the companion archive and embedded in Appendix A. Preserve matching files; reconcile conflicting existing versions explicitly. Add a concise pointer to the existing root `AGENTS.md` without overwriting unrelated rules.

Use the installed client's supported configuration schema. Current official documentation describes project custom-agent TOML files and explicit model/effort fields; verify version compatibility and account availability. [S1–S3] The intended model identifiers are `gpt-6-astra`, `gpt-5.6-luna`, and `gpt-5.6-sol`; resolve locally rather than silently substituting.
...
```toml
# .codex/agents/luna-programmer.toml
...
### 3.3 Short startup rehearsal

Before production changes depend on this system, demonstrate in disposable task branches:

1. A deliberately faulty toy candidate is rejected by the designated Sol reviewer.
2. Its Luna owner receives the findings, makes a corrective local commit, retests, and resubmits without another owner instruction.
...
Keep this rehearsal small. It is a workflow check, not an invitation to build a general-purpose approval framework.

---

## 4. Commit, review, and integration protocol

This section summarizes the binding shared role protocol in Appendix A; use the role files for detailed duties.

Each Luna task gets its own Git worktree and branch from an assigned base. Git worktrees isolate checked-out files, not GPU state, caches, installed packages, ports, or every repository-level setting. [S4] Allocate separate output directories and resource ownership as well.
...
Luna remains accountable until integration or reassignment. It must automatically address justified findings and resubmit. Two rounds stalled on the same cause without new evidence trigger separate specialist or owner-chat diagnosis; demonstrated progress can justify more rounds. Sol corrects mistaken findings when a rebuttal proves them wrong.

Astra alone controls the integration branch. Preserve the accepted base when a candidate fails. When the base is unchanged, integration checks can run on the approved candidate before fast-forward. When the base moved, create an isolated trial merge, route required fixes to a worker, obtain Sol review of the combined candidate, and test that exact state. No hidden cleanup, squash, or cherry-pick into an unreviewed source snapshot.

Astra edits to production source are subject to the same independent code-review gate. A separate Sol specialist is also reviewed. Neither deadlines nor repeated rejections waive blockers.
...
**Evidence:** multi-view clip and orbit diagnostic of the same estimated boxes, neighboring simultaneous-object test, duplicate-ID/merge test, occlusion test, selected label screenshots at 4K and 1080p, and object-to-evidence provenance.
...
For the production sensor recording, target native 3840 × 2160 RGB at 30 Hz as well; record the effective settings rather than assuming the output film resolution describes the sensor. Diagnostic captures may be smaller and must be labeled. Any required lower-resolution production sensor input needs an explicit recorded decision, matching calibration, and disclosure in the evidence; a cinematic 4K rerender does not turn a lower-resolution sensor capture into a 4K measurement.
...
Create preview, look-development, and final profiles. Preview can be lower resolution/samples for iteration and must be labeled. Final is native 4K. Compare a small set of increasing sample counts on representative dark shelving, glossy floors, fine packaging print, hair/cart wire geometry, and emissive lines. Choose a demonstrated convergence point rather than an arbitrary maximum.
...
Measure representative frame sizes and timings, project disk use for master/layers/cache/video, and check free space before starting. Use a documented safety reserve appropriate to the workstation; stop new writes safely before exhausting the disk. Report the actual projected requirement rather than assuming 4K intermediates are small.
...
Test at 4K and a normal 1080p viewing size. Check black frames, clipped/cropped titles, visible banding, crushed shadows, washed-out colors, timeline discontinuities, and start/end rounding errors. Streamable playback and codec compatibility should be verified on the available local player.
...
Astra decomposes the specification into small reviewable tasks, not one enormous assignment per subsystem. Suggested ownership groups:

| Workstream | Typical owner | Scope | Critical boundary |
|---|---|---|---|
| Environment and asset preparation | Luna | Catalog acquisition, geometry/UV/material fixes, scene construction, visual samples | No estimator truth injection; physical changes invalidate capture |
| Camera and character presentation | Luna; Sol specialist for difficult integration | Trajectory/animation replay, optics, shot composition, timeline | Sensor camera versus cinematic camera must remain explicit |
| Capture and calibration | Luna for bounded fixes; separate Sol specialist when needed | Sensor timing, transforms, serialization, deterministic recording, validation | One owner for frame/time contracts |
| Mapping and persistent state | Separate Sol specialist or qualified Luna with mandatory Sol review | Offline replay, map versions, correction handling, static/dynamic separation | Observed geometry and correct map-frame semantics |
| Product perception and object geometry | Luna with focused Sol implementation help | Instances, recognition, temporal association, depth support, 3D bounds | No distance-only identity collapse or hidden ground truth |
| Presentation and exports | Luna | Data-driven points/rays/bounds/cards, compositing, resumable frame outputs, audio/video | Graphics do not repair estimates |
| Evidence and tests | Luna tasks independent of implementation where practical | Reproducers, integration fixtures, comparisons, provenance and review index | Tests retain frozen meaning; no fabricated passes |
...
|---|---|---|---|
| **G00 — real baseline and agent workflow** | Correct local repo is identified; original references open; small baseline render/capture runs; model/effort/permissions are verified or explicitly unresolved; worker→Sol rejection/revision/approval rehearsal works | Runtime inventory, exact baseline SHA, source/reference hashes, rehearsal packets, actual frame, test commands, access/publication status | First gate. No production reliance on unavailable capabilities or fabricated review routing |
...
| **G08 — native 4K final candidate** | Exactly 1,350 valid native 4K frames encode to 45 seconds at 30 fps; converged quality, correct color/audio, resumability, complete reference and regression checks | Final film and silent copy; master-frame manifests; ffprobe or equivalent metadata; interruption test; full-resolution crops; end-to-end logs; Sol final integration verdict | G07 accepted internally; no unresolved hard technical/visual blockers, no stale mismatched frame reuse |
...
The owner wants to use this chat as a high-capability design/diagnosis/review resource. Use it for decisions with high rework cost and for judging actual results against the storyboard. Do not pay Astra to repeatedly reconsider all architecture when a focused evidence packet can obtain the needed decision here.

Routine implementation and every ordinary revision stay in the Luna↔Sol loop. Astra performs compact routing and integration rather than redoing Sol's entire review. A platform requiring parent-mediated handoffs still needs Astra turns; do not pretend local subagents have unsupported direct messaging or an automatic connection to this chat.
...
2. One native 4K/30 fps 45-second film matching the 12-shot sequence, plus silent and lightweight review copies. Retain the lossless frame master locally with its manifest.
...
## Appendix A. Embedded role documents

The following four blocks reproduce the previously prepared role instructions. They are included for recovery when this Markdown is supplied without the companion archive. Write each block to the stated path only when it is missing or after explicitly reconciling an existing version. Do not overwrite unrelated repository instructions.

The role package's original "proposed" and "does not authorize implementation" status described its earlier standalone delivery. This master specification now supplies the implementation assignment and acceptance scope. The role protocol remains the shared operating contract; it does not itself start agents or override client permissions.

Astra must not load all embedded role bodies into every task context. After installation, each agent reads the shared README, its own role, and the task-relevant specification sections.
