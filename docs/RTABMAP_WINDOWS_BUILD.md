...
Run `scripts/setup_rtabmap_windows.ps1 -Build` after setting `ISAACSIM_ROS_WORKSPACE` or passing `-RosWorkspace`. The script clones missing sources, checks out those revisions, applies `patches/rtabmap_ros_windows.patch`, verifies MSVC/SDK discovery, and builds through the active Pixi manifest. It verifies:
...
The patch is limited to Windows export/import visibility, `NOMINMAX`/`ERROR` macro collisions, and the MSVC-compatible covariance buffer API. It does not alter RTAB-Map algorithms.
