...
- Exact review of successor `1be5e41d0ae4971baa2dd7ab43e0257ebf31488f` (tree `45fc1d2215377bc0b3818f55059972086aa1f962`) accepted the documentation fixes but returned **REQUEST_CHANGES** with blockers `RGB-1BE-F01` (claim inputs and timestamps were not all pinned and validated) and `RGB-1BE-F02` (the raw six-sample sheet was not pinned before writes).
- Successor `76362124067b548a784b5e04161c82a4fc110c74` (tree `bc8f093a68742cf268e3145380221f04b92e3169`) added fail-closed validation of all 12 inputs and the index/capture/configuration/SLAM/perception bindings and pinned the sheet before output writes. Its exact review returned **REQUEST_CHANGES** with blocker `RGB-763-F01`: validated mutable paths were reopened after validation, allowing later bytes to supply published claims or media.
- Candidate `80ddc4a45e0476fdd73dad79f7bb0f1b95200107` (tree `35a89207cfab2c9fe08fd8dbf09052be7f6298dc`) added verified snapshots, staged generation, stable-output checks, final input rehashing, and manifest-last rollback for ordinary failures. Exact review returned **REQUEST_CHANGES** with `RGB-80D-F01` through `RGB-80D-F03`: sensitive transaction scratch was addable and claimed unsupported Windows privacy, the builder identity was not externally pinned through publication, and process termination between replacements lacked restart recovery.
- Reviewed successor `5129e23334d97aa915a3dd88eed7afeba95191b9` (tree `51935c2867b7ffb71686e78f25fee745e6d900cc`) closes those findings. Scratch uses the Git-ignored same-volume `runs/.rgb-evidence-transactions/` namespace with inherited host ACLs and no restrictive privacy claim; startup binds one exact tracked builder blob and the original path is rehashed before publication; process-termination recovery backs up the old manifest first, publishes four files, and publishes the new manifest last. Windows sudden power loss and filesystem-cache durability are outside this claim. Independent exact builder/transaction review returned **APPROVE** with zero blockers after ref-race, 10 rename-boundary, 22 crash/journal-lag, mixed-presence, pre-`PREPARED`, rollback, junction, ignore, and output-binding checks. This remains scoped RGB visual evidence; external combined-candidate and owner acceptance, seven final 1080p technical clips, and the 1,350-frame final film remain pending.
