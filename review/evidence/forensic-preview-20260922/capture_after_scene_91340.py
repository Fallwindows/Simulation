...
SOURCE_ROOT = Path(r"C:\Users\suyog\.codex\worktrees\forensic-scene\Simulation")
SCENARIO_PATH = SOURCE_ROOT / "config/scenarios/walking_baseline.yaml"
EXPECTED_COMMIT = "91340cfc47a02f18341ddc5a80963337ed6aa394"
EXPECTED_TREE = "1db83f7bc0a763bc01512f447c6bb1ddc8276e88"
OUTPUT_TAGS = {0.0: "t000000ms", 8.0: "t008000ms"}
...
def _validate_inputs() -> tuple[str, str, dict[str, str], dict[str, str], dict[str, str]]:
    ...
    usda_paths = tuple(sorted((SOURCE_ROOT / "assets/retail/usd").glob("*.usda")))
    texture_paths = tuple(sorted((SOURCE_ROOT / "assets/retail/textures").glob("*.png")))
    if len(usda_paths) != 34 or len(texture_paths) != 34:
        raise RuntimeError(f"unexpected asset inventory: {len(usda_paths)} USDA, {len(texture_paths)} textures")
    asset_relatives = tuple(path.relative_to(SOURCE_ROOT).as_posix() for path in (*usda_paths, *texture_paths))
    dirty = _git("status", "--short", "--", *BASE_INPUT_PATHS, *asset_relatives)
    if dirty:
        raise RuntimeError(f"capture inputs differ from HEAD:\n{dirty}")
    input_hashes = {relative: _sha256(SOURCE_ROOT / relative) for relative in BASE_INPUT_PATHS}
    usda_hashes = {path.relative_to(SOURCE_ROOT).as_posix(): _sha256(path) for path in usda_paths}
    texture_hashes = {path.relative_to(SOURCE_ROOT).as_posix(): _sha256(path) for path in texture_paths}
    return head, tree, input_hashes, usda_hashes, texture_hashes


def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Capture one approved forensic-scene RGB still")
    parser.add_argument("--time-s", type=float, required=True, choices=tuple(OUTPUT_TAGS))
    return parser.parse_args()


def main(capture_time_s: float) -> None:
    tag = OUTPUT_TAGS[capture_time_s]
    output_path = EVIDENCE_DIR / f"after_scene_91340_{tag}.png"
    record_path = EVIDENCE_DIR / f"after_scene_91340_{tag}.json"
    if output_path.exists() or record_path.exists():
        raise FileExistsError("refusing to overwrite existing forensic evidence")

    head, tree, input_hashes, usda_hashes, texture_hashes = _validate_inputs()
    ...
    try:
        ...
        if not output_path.is_file():
            raise RuntimeError(f"expected PNG was not written: {output_path}")

        script_hash = _sha256(Path(__file__).resolve())
        record = {
            "status": "complete",
            "purpose": "approved forensic-scene comparable after frame",
            "source": {
                "commit": head,
                "tree": tree,
                ...
                "authored_usda_sha256": usda_hashes,
