...
Consequently `rgb_smoke.mp4`, recorder metadata, camera-info JSON, and the requested PNG extraction were not produced. The exact recorder stderr/stdout and simulator logs are in `rgb-smoke-logs/`. This is a genuine environment blocker for the existing recorder, with no package install or repair attempted. The simulator status also recorded a ROS executor `InvalidHandle` traceback during teardown in the same run; despite that traceback, the wrapper exited `0` and the process shut down.
