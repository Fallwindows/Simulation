...
def _validate_capture_manifest(path: Path) -> None:
    data = _json(path)
    from simulator.capture.manifest import validate_capture_manifest_artifacts, validate_capture_manifest_hash

    validate_capture_manifest_hash(data)
    validate_capture_manifest_artifacts(data, path.parent)
    bag = data.get("bag")
    if not isinstance(bag, dict) or bag.get("topics") != list(REQUIRED_CAPTURE_TOPICS):
        raise ValueError("capture manifest must contain the exact five manifest-v2 raw topics")
    if "/sim/camera/rgb/camera_info" in bag.get("topics", []):
        raise ValueError("configured camera_info is an artifact, not a raw bag topic")
