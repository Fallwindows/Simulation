...
def _verify_configured_camera_info(camera_info_path: Path, capture_dir: Path, inventory: dict[str, str]) -> None:
    ...
    if camera_info.get("provenance") != "configured_intrinsics" or camera_info.get("observed_ros_message") is not False:
        raise ValueError("RGB receipt requires the capture producer's configured camera intrinsics")
...
def _verify_rgb_timing(
    ...
    expected_dimensions = (int(camera_info["width_px"]), int(camera_info["height_px"]))
...
def emit_rgb_bundle(
    ...
    if rgb.get("camera_info_provenance") != "configured_intrinsics":
        raise ValueError("capture manifest does not declare configured camera intrinsics provenance")
    _verify_configured_camera_info(source_paths["camera_info"], capture_dir, capture_inventory)
    _verify_rgb_timing(
    ...
    if (
        video_probe["width"] != int(_json(source_paths["camera_info"])["width_px"])
        or video_probe["height"] != int(_json(source_paths["camera_info"])["height_px"])
    ...
    view_receipt = {
        ...
        "producer_source_sha256": sha256_path(root / spec.view_producer_source_path),
        "role": "rgb",
    ...
    role = {
        ...
        "producer_source_sha256": sha256_path(root / spec.producer_source_path),
        "capture_id": str(capture["capture_id"]),
        "map_version": None,
        "object_state_version": None,
        "acceptance": {
